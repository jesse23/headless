...
const formatNode = (node: Node, level = 0): Node => {
  /*
    ...
    */
  const tmpNode = (
    level && node.parentNode ? node.parentNode : node
...
const applyValueInternal = (
  ...
  if (paths.length === 0) {
    ...
    return {
      ...prev,
      [paths[0]]: applyValueInternal(paths.slice(1), value, curr),
    };
  }
};
 ...
 */
export const createPartialStore: GetPartialStoreFn = (
  store: Store,
  path: string
) => {
